...
| Message | Cause |
|---------|-------|
| Unable to read member data from Internet - using cached copy.<br/>Member ranks may be out of date. | When the program starts it reaches out over the Internet to fetch the current rankings from MEMPAD. This message is a warning, indicating that MEMPAD cannot be reached (is your Internet connection OK?).<br/>If the program has previously contacted MEMPAD it will have cached a copy of the last ranking list obtained from MEMPAD and will use that copy instead.<br/>This means that any recent changes to player rankings will not be used when generating stratified results. |
...
| Unable to start Help server | Something is preventing the program from running the local Help server.<br/>Help pages will not be available.<br/>Get your IT person to check nothing is using port TCP/8080. |
